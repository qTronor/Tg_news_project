...
**Стек:** FastAPI + SQLAlchemy (async) + asyncpg + python-jose (JWT) + passlib (bcrypt) + slowapi (rate limiting)
...
**Таблицы (5 штук):**
- `users` (id UUID, email, username, password_hash bcrypt-12, role admin/user, is_active, timestamps)
...
- `POST /api/auth/register` — регистрация, возвращает JWT пару
...
│   ├── security.py      — hash_password, verify, create_access/refresh_token, decode
...
**Стек:** Next.js 16 (App Router) + TypeScript + Tailwind CSS v4 + Recharts + Cytoscape.js + Framer Motion + TanStack Query + next-themes + jose

### Файловая структура frontend/src/:

**Страницы (12 маршрутов):**
- `/` — Dashboard: KPI-карточки, sentiment area chart, top entities, top topics, channel bar chart
- `/login` — Страница входа/регистрации (без sidebar), animated, с toggle между режимами
...
- `components/layout/sidebar.tsx` — Анимированный sidebar, role-based: user видит 6 пунктов, admin +2 (Каналы, Аудит) с разделителем ADMIN
- `components/layout/header.tsx` — Time range presets, Demo/Live, **кнопка смены языка (RU↔EN)**, тема, user dropdown с logout
- `components/layout/app-shell.tsx` — SidebarContext, скрывает sidebar на /login
...
- `lib/auth.ts` — Auth API client: login, register, logout, getProfile, editMessage, channels, reactions, audit. JWT управление: localStorage, автоматический refresh до истечения, singleton promise, auth:logout event на 401
...
- `lib/i18n.ts` — Двуязычная система (RU/EN): ~120 ключей, createContext, useTranslation hook, getTranslator
...
- Все 12 маршрутов компилируются и отдают 200
...
- Добавить поддержку мобильных устройств (responsive)
- Написать тесты (Jest/Playwright)
