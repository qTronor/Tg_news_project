param(
    [string]$InputPath = "docs/gpt_topics_clustered_2026_04_21_v1.jsonl",
...
$allowedEventTypes = @(
    "policy", "markets", "company", "conflict", "crime",
    "technology", "public_health", "society", "media", "other"
)
...
Get-Content -LiteralPath $InputPath -Encoding UTF8 | ForEach-Object {
    ...
    foreach ($field in @(
        "request_id", "event_id", "channel", "message_id", "raw_message_id",
        "preprocessed_message_id", "run_id", "cluster_id", "public_cluster_id",
        "cluster_probability", "bucket_id", "topic_label", "event_type"
    )) {
    ...
    if ($null -ne $obj.event_id -and $seen.ContainsKey([string]$obj.event_id)) {
        $errors.Add("line ${lineNumber}: duplicate event_id $($obj.event_id)")
    } elseif ($null -ne $obj.event_id) {
        $seen[[string]$obj.event_id] = $true
    }

    if ($null -ne $obj.cluster_id -and [int]$obj.cluster_id -lt -1) {
        $errors.Add("line ${lineNumber}: invalid cluster_id $($obj.cluster_id)")
    }
    if ($null -ne $obj.cluster_probability) {
        $prob = [double]$obj.cluster_probability
        if ($prob -lt 0 -or $prob -gt 1) {
            $errors.Add("line ${lineNumber}: cluster_probability out of range $prob")
        }
    }
    if ($null -ne $obj.event_type -and $allowedEventTypes -notcontains [string]$obj.event_type) {
        $errors.Add("line ${lineNumber}: invalid event_type $($obj.event_type)")
    }
    if ($null -ne $obj.public_cluster_id -and $null -ne $obj.run_id -and $null -ne $obj.cluster_id) {
        $expected = "$($obj.run_id):$($obj.cluster_id)"
        if ([string]$obj.public_cluster_id -ne $expected) {
            $errors.Add("line ${lineNumber}: public_cluster_id mismatch expected ${expected}")
        }
    }
}
...
$sqlPath = Join-Path $importDir "gpt_topic_clusters_import.sql"
$containerJsonlPath = "/tmp/gpt_topics_clustered_2026_04_21_v1.jsonl"

docker cp $InputPath "${PostgresContainer}:${containerJsonlPath}"
...
\copy gpt_topic_cluster_raw (line) FROM '$containerJsonlPath';
