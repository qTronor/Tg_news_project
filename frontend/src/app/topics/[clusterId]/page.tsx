...
export default function TopicDetailPage({ params }: { params: Promise<{ clusterId: string }> }) {
  ...
  return (
    <>
      <Header title={`Topic: ${detail.label}`} />
      <PageTransition>
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  {detail.message_count} messages &middot; {detail.channel_count} channels &middot; Avg sentiment: {detail.avg_sentiment.toFixed(2)}
                </p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card>
              ...
              <div className="flex items-center justify-center gap-4 mt-2 text-xs">
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-positive" /> Positive</span>
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-neutral-sentiment" /> Neutral</span>
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-negative" /> Negative</span>
              </div>
            </Card>
